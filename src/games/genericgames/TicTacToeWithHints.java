...
public class TicTacToeWithHints extends TicTacToe{
    ...
    public TicTacToeWithHints(Player j1 , Player j2){
        super(j1,j2);
	}
    ...
    public List<Integer> hints(){
        ...
        Player joueur = getCurrentPlayer().equals(super.joueur1) ? super.joueur2 : super.joueur1;
    ...
    public String situationToString(){
 		String chaine = "  0 1 2\n";
        List<Integer> hints = hints();// Récupérer les hints pour le joueur courant
		System.out.println(hints);
		for(int i=0;i<=2;i++) {
			chaine += i;
			for(int j=0;j<=2;j++) {
				Player joueur=super.tab[i][j];
