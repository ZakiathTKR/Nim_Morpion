...
public class TicTacToeWithHints extends TicTacToe{
    ...
    public ArrayList<Integer> hints(){
        ...
        String joueur = getCurrentPlayer().equals(this.joueur1) ? this.joueur2 : this.joueur1;
		// Tester tous les coups possibles
        for(int i=0;i<=2;i++){
            for(int j=0;j<=2;j++){
                if(tab[i][j] == null){ // si la case est vide
 		            tab[i][j] = joueur; //simuler un coup de l'adversaire
					//if(getWinner())
                	...
                	tab[i][j] = null;//Anuler le coups simulé
				}
            }
        }
        return hints;
    ...
    public String situationToString(){
		...
		for(int i=0;i<=2;i++) {
            ...
            // Si la case contient un coup menant à la victoire, afficher "!"
				...
				joueur.equals(this.joueur1) ? "X": "O");  // si joueur différent de null "X" sinon "0"
